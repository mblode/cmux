...
final class Workspace: Identifiable, ObservableObject {
    ...
    nonisolated static func bonsplitChromeColors(
        ...
        // A user-configured pane border wins; otherwise fall back to the fixed
        // Linear border for the current scheme (tab separators, the bar's bottom
        // hairline, pane dividers) rather than a translucent terminal-derived tone.
        let borderHex = PaneChromeSettings.resolvedPaneBorderHex(
            configuredHex: paneBorderColorHex,
            fallback: SidebarChromeColors.borderHex(for: colorScheme)
        )
